...
const S = { parents: {}, orders: [], createCalls: 0, failSkuOnce: null, dropResponseOnce: false, draftN: 1000, orderN: 5000 };
...
function handle(query, vars) {
  ...
  if (query.includes('mutation CreateParent')) {
    const o = vars.order;
    const gross = o.lineItems.reduce((n, li) => n + cents(li.priceSet.shopMoney.amount) * li.quantity, 0);
    const ship = o.shippingLines.reduce((n, sl) => n + cents(sl.priceSet.shopMoney.amount), 0);
    const tax = (o.taxLines || []).reduce((n, tl) => n + cents(tl.priceSet.shopMoney.amount), 0);
    const disc = o.discountCode && o.discountCode.itemFixedDiscountCode
      ? cents(o.discountCode.itemFixedDiscountCode.amountSet.shopMoney.amount) : 0;
    const id = 'gid://shopify/Order/' + (++S.draftN);
    const rec = { id, name: '#' + S.draftN, input: o, options: vars.options, total: gross - disc + ship + tax,
                  financialStatus: 'PENDING', cancelledAt: null, sent: null };
    S.parents[id] = rec;
    return { orderCreate: { userErrors: [], order: { id, name: rec.name,
      totalPriceSet: { shopMoney: { amount: (rec.total / 100).toString() } },
      paymentCollectionDetails: { additionalPaymentCollectionUrl: 'https://shop.test/pay/' + S.draftN } } } };
  }
  if (query.includes('orderInvoiceSend')) {
    const p = S.parents[vars.id];
    p.sent = vars.email || true;
    return { orderInvoiceSend: { userErrors: [], order: { id: p.id, name: p.name } } };
  }
  if (query.includes('ParentStatus')) {
    const p = S.parents[vars.id];
    return { order: p ? { id: p.id, name: p.name, displayFinancialStatus: p.financialStatus, cancelledAt: p.cancelledAt,
      paymentCollectionDetails: { additionalPaymentCollectionUrl: 'https://shop.test/pay/x' } } : null };
  }
...
const expectShipping = withQuotes.reduce((n, r) => n + r.quote.cents.shipping, 0);
const expectTax = withQuotes.reduce((n, r) => n + r.quote.cents.tax, 0);
...
const inv = Object.values(S.parents)[0];
ok(inv.total === expectTotal, `invoice total = sum of per-destination quotes ($${amt(expectTotal)})`);
ok(inv.input.financialStatus === 'PENDING', 'invoice is created unpaid, awaiting payment');
ok(inv.input.lineItems.every(l => l.variantId && !l.title), 'invoice line items are REAL product variants, not custom lines');
ok(inv.input.lineItems.length === 3, 'one line per product');
const ginLine = inv.input.lineItems.find(l => l.variantId === V['BSKT-053'].id);
ok(ginLine.quantity === 2 && ginLine.priceSet.shopMoney.amount === '249.95', 'a product two people get is one line, quantity 2, at its real price');
ok(inv.input.shippingLines.length === 1 && cents(inv.input.shippingLines[0].priceSet.shopMoney.amount) === expectShipping,
   'shipping is a real shipping line, not a product');
const taxSum = inv.input.taxLines.reduce((n, t) => n + cents(t.priceSet.shopMoney.amount), 0);
ok(inv.input.taxLines.length > 0 && taxSum === expectTax, 'tax is real tax lines, per jurisdiction, totalling what the recipients were quoted');
ok(inv.input.taxLines.every(t => t.title && typeof t.rate === 'number'), 'each tax line names its jurisdiction and rate');
ok(cents(inv.input.discountCode.itemFixedDiscountCode.amountSet.shopMoney.amount) === expectTotalDiscount,
   'discount is a real discount at the exact amount quoted');
ok(inv.options.inventoryBehaviour === 'BYPASS', 'the invoice holds no stock — inventory moves on the recipient orders');
ok(inv.input.tags.includes('bulk-parent') && inv.input.tags.includes('batch-B20260921-INV1'), 'invoice tagged bulk-parent + batch');
ok(inv.input.poNumber === 'PO-77', 'PO number carried to the invoice');
ok(!inv.input.shippingAddress, 'no shipping address on the invoice, so nothing tries to ship it');
ok(!c1.data.batch.log.some(l => /WARNING/.test(l.msg)), 'no total or tax mismatch warnings');

const dup = await call('create-batch', { body: { id: 'B20260921-INV1', buyer, pricing, recipients: withQuotes } });
ok(dup.status === 409 && Object.keys(S.parents).length === 1, 'second create of same batch is refused; still one invoice');
...
ok(Object.values(S.parents).filter(d => d.input.tags.includes('batch-B20260921-DBL1')).length === 1, 'simultaneous creates: exactly one invoice in Shopify');
...
ok(si.status === 200 && inv.sent, 'invoice emailed through Shopify');
...
inv.financialStatus = 'PAID';
st = await call('batch-status', { method: 'GET', qs: '?id=B20260921-INV1' });
ok(st.data.batch.status === 'paid' && st.data.batch.parent.orderName === inv.name, 'paid invoice flips the batch to paid');
...
ok(attr('Bulk Parent Order') === inv.name && attr('Gift Message') === 'Great year, Ben' && attr('Adult Signature Required') === 'Yes', 'parent ref, gift message, adult signature on the child');
...
const parentsBefore = Object.keys(S.parents).length;
...
const pd = Object.values(S.parents)[parentsBefore];
ok(pd.input.tags.includes('bulk-source-customer') && pd.input.email === 'jo@acme.test', 'invoice tagged as a customer order, billed to the buyer');
ok(pd.sent && pd.sent.customMessage && pd.sent.customMessage.includes(sub.data.statusUrl), 'invoice email carries the status link');
